...
public class Validation {
    ...
    public String getWrongSign(String expression){
        if(expression == null || expression.equals(""))return "wrongExpression";
        if(!Character.isDigit(expression.charAt(0))) return "wrongBeginExpression";
        if(!Character.isDigit(expression.charAt(0)) && !Character.isDigit(expression.charAt(1))) return "wrongBeginExpression";
        ...
        if (!Character.isDigit(expression.charAt(expression.length() - 1))){
            for(i = 0; i < expression.length(); i++) {
                ...
                if(expression.charAt(i) != '_' && expression.charAt(i) != '.' && !Character.isDigit(expression.charAt(i))) operator = operator + 1;
            }
            if (delimeter != 0 && delimeter - 1 != operator) return "wrongOperation";
        }
    ...
    public String checkBracets(String expression){
        ...
        if (!stack.empty()) return  "wrongBrackets";

        for (int i = 0; i < expression.length(); i++) {
            if (expression.charAt(i) == '(') openBracket = openBracket + 1;
            if (expression.charAt(i) == ')') closeBracket = closeBracket + 1;
            }
        if(!openBracket.equals(closeBracket)) return  "wrongBracketsBalance";

        return "Brackets checked";
    }

    public String getInfinity(String expression){
